...
var codeToHTTPStatusMap = map[errorCode]int{
	...
	EALREADYEXISTS:    http.StatusBadRequest,
	EPERMISSIONDENIED: http.StatusForbidden,
	EUNAUTHENTICATED:  http.StatusUnauthorized,
	ECANNOTDECODE:     http.StatusBadRequest,
	ECANNOTENCODE:     http.StatusInternalServerError,
	EBEHAVIOUR:        http.StatusConflict,
	EUNSUPPORTED:      http.StatusMisdirectedRequest,
	ETEST:             499,
}
...
func ErrorMessageDefault(err error, def string) string {
	...
	target := &Error{}
	if errors.As(err, &target) {
		if target.Message != "" && target.Err == nil {
		...
		buf.WriteString(target.Message)
		buf.WriteString(": ")
		buf.WriteString(ErrorMessageDefault(target.Err, def))
		return buf.String()
	}
